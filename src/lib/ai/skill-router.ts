...
export function buildSkillRouterPrompt(
  ...
  return [
    ...
    'Resolve the CURRENT turn as part of the recent conversation, not as an isolated sentence. Short follow-ups such as “esse”, “o segundo”, “top”, a colour, a size, “mais barato”, “outro”, “sim”, “não”, “quero esse” or “e em preto?” inherit the active customer goal unless the customer clearly changes topic.',
    'A specialised skill that was materially relevant to the active goal remains relevant for an elliptical follow-up, refinement, rejection, comparison or confirmation within that same goal. Do not drop it merely because the newest message is short or contains no explicit product/service noun.',
    'The newest customer message still has priority over stale context. A clear correction, rejection or topic change must immediately replace the old interpretation.',
    ...
    'Select none only when the current conversational move genuinely requires no configured specialised objective after resolving its references against recent turns. A greeting or unrelated ordinary conversation may select none; an in-progress specialised task should not become skill-less just because the customer replied briefly.',
