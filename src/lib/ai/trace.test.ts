...
describe('agent trace', () => {
  it('one-shot writer remains privacy-reduced and marks the run complete', async () => {
    ...
    await recordTrace(db, {
      accountId: 'acct-1', agentId: 'agent-1', conversationId: 'conv-1', turnId: 'turn-1', startedAt: 100,
      intent: 'sales', modelTier: 'smart', finalAction: 'reply', totalMs: 321.4, memoryMatchCount: 2,
      guardrailViolations: [], toolCalls: [{ name: 'search_catalog', ms: 88.7, succeeded: true }],
    })
    expect(inserts).toHaveLength(1)
    expect(inserts[0]).toMatchObject({
      table: 'agent_traces',
      payload: {
        account_id: 'acct-1', agent_id: 'agent-1', conversation_id: 'conv-1', turn_id: 'turn-1',
        status: 'completed', final_action: 'reply', total_ms: 321, memory_match_count: 2,
  ...
  it('creates a running parent, ordered observable steps and one final update', async () => {
    const { db, inserts, updates } = liveTraceDb()
    let now = 1000
    const trace = createAgentTraceCollector({
      db, accountId: 'acct-1', agentId: 'agent-1', conversationId: 'conv-1', turnId: 'turn-1',
      inboundMessageId: 'wamid-1', provider: 'openai', model: 'model-1', now: () => now,
    })
    ...
    expect(parent?.payload).toMatchObject({
      account_id: 'acct-1', conversation_id: 'conv-1', turn_id: 'turn-1', status: 'running',
      provider: 'openai', model: 'model-1', inbound_message_id: 'wamid-1',
    })
    const stepInserts = inserts.filter((row) => row.table === 'agent_trace_steps').map((row) => row.payload)
    expect(stepInserts.map((row) => row.type)).toEqual([
      'message_received', 'intent_classified', 'memory_retrieved', 'tool_finished', 'guardrail_checked', 'handoff_triggered',
    ])
    expect(stepInserts.map((row) => row.sequence)).toEqual([0, 1, 2, 3, 4, 5])
    const finalUpdate = updates.filter((row) => row.table === 'agent_traces').at(-1)
    expect(finalUpdate?.payload).toMatchObject({
      status: 'blocked', final_action: 'handoff', intent: 'faq', model_tier: 'fast', memory_match_count: 2,
  ...
  it('does not throw when observability storage fails', async () => {
    ...
    const trace = createAgentTraceCollector({ db, accountId: 'acct-1', agentId: 'agent-1', conversationId: 'conv-1' })
    expect(() => trace.recordToolCall({ name: 'search_catalog', ms: 1, succeeded: true })).not.toThrow()
