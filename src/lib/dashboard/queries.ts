...
// --- 1b. Priorities row -------------------------------------------------
// Three counts that already exist elsewhere in the app, surfaced
// together as the dashboard's "needs attention today" row instead of
// buried in a nav badge / a single log line in the activity feed.

export async function loadPriorities(db: DB): Promise<PriorityCounts> {
  const dayAgo = daysAgoStart(1).toISOString();
  const weekFromNow = new Date(Date.now() + 7 * 86_400_000)
    .toISOString()
    .slice(0, 10);
  const today = new Date().toISOString().slice(0, 10);

  const [awaiting, automationErrors, dealsClosingSoon] = await Promise.all([
    // Same "unread_count > 0" signal the Inbox nav dot uses — reused
    // rather than re-deriving "awaiting reply" from message pairs.
    db
      .from('conversations')
      .select('id', { count: 'exact', head: true })
      .gt('unread_count', 0),
    ...
    db
      ...
      .lte('expected_close_date', weekFromNow),
  ]);

  return {
    awaitingReply: awaiting.count ?? 0,
    automationErrors: automationErrors.count ?? 0,
    dealsClosingSoon: dealsClosingSoon.count ?? 0,
  };
}

// --- 1c. AI agent activity today ----------------------------------------
// Reads the same `agent_traces` table the Agentes → Fluxo ao vivo panel
// already subscribes to in realtime — this is a same-day summary of it,
// not a new data source. Deliberately excludes cost/tokens: that figure
// is gated to admin-capable users elsewhere (`/api/ai/usage`), so it's
// left off this always-visible dashboard widget rather than duplicating
// that access check here.

export async function loadAgentActivity(db: DB): Promise<AgentActivitySummary> {
  const todayStart = startOfLocalDay().toISOString();

  const [configRes, tracesRes] = await Promise.all([
    db.from('ai_configs').select('is_active, agent_name').maybeSingle(),
    db
      .from('agent_traces')
      .select('conversation_id, status, total_ms')
      .gte('created_at', todayStart),
  ]);

  const traces = (tracesRes.data ?? []) as {
    conversation_id: string;
    status: string;
    total_ms: number | null;
  }[];

  const conversationsToday = new Set(traces.map((t) => t.conversation_id)).size;
  const handoffsToday = traces.filter((t) => t.status === 'handoff').length;
  const completed = traces.filter(
    (t) => t.status === 'completed' && t.total_ms != null
  );
  const avgLatencyMs =
    completed.length === 0
      ? null
      : completed.reduce((sum, t) => sum + (t.total_ms ?? 0), 0) /
        completed.length;
