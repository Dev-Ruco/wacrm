-- Prevent cross-tenant reads through the SECURITY DEFINER availability RPC.
-- service_role is used by the server-side agent tool path; authenticated users
-- may call the RPC only for accounts they actually belong to.
...
declare
  ...
  v_exception record;
begin
  if auth.role() <> 'service_role' and not wacrm.is_account_member(p_account_id) then
...
revoke all on function wacrm.check_operational_availability(uuid,timestamptz,timestamptz,uuid,uuid)
  from public, anon;
grant execute on function wacrm.check_operational_availability(uuid,timestamptz,timestamptz,uuid,uuid)
